...
def print_inventory_items(items):
    ...
    item_list = list_of_items(item_list)
    print("You have %s.\n"%(item_list))
...
def print_menu(exits, room_items, inv_items):
    ...
    for item in room_items:
        print("TAKE %s to take %s."%(item["id"].upper(), item["name"]))

    for item in inv_items:
        print("DROP %s to drop %s."%(item["id"].upper(), item["name"]))
...
def main():
    ...
    while True:
        ...
        if current_room["name"] == "Reception" and len(current_room["items"]) == 6:
            print("YOU WIN!")
            break
